...
애초에 쿠키 자체가 request 시에 자동으로 서버로 전송되나, httponly 옵션을 주면 자바스크립트 단계에서 접근이 불가능하여, xss 공격에 내성이 생기는 것이다.  
로컬스토리지의 경우 xss 공격에 더 취약한 편이니, 이런 점에서 서버 측에서 httponly 옵션과 path를 사용하여 특정 링크(재발급)로만 쿠키가 전송되도록 하는 것이 제일 안전한 방법인 것이다.
물론 서버 메모리나 db에 저장하면 더 안전하겠지만, 그 순간 이미 세션의 개념으로 취급해야 한다고 봐야기 때문에, jwt 토큰 방식만의 장점을 취하면서 제일 안전한 것은 옵션을 적절하게 설정해놓은 쿠키라고 볼 수 있을 것이다.
...
로컬 변수의 경우 xss 공격과 csrf 공격으로 탈취하기가 더 어렵기 때문이다.  
